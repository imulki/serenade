...
class FramewiseDecoder:
    ...
    def decode(self, pred, audio=None, f0=None):
        ...
        pitches, intervals = _decode_notes(
            onsets, f0, self.pitch_sum, offsets=offsets, frames=frames
        )
...
def _decode_notes(onsets, f0, pitch_sum, offsets=None, frames=None):
    ...
    if offsets is not None:
        offset_diff = (
            torch.cat([offsets[:1], offsets[1:] - offsets[:-1]], dim=0) > 0
        ).float()
    if frames is not None:
        frames_quantized = (frames >= 0.5).float().to("cpu")
        frame_diff = (
            torch.cat(
                [frames_quantized[:-1] - frames_quantized[1:], frames_quantized[-1:]],
                dim=0,
            )
            == 1
        ).float()
    ...
    for i, nonzero in enumerate(onset_diff_nonzero):
        ...
        if pitch_sum == "median":
            ...
            pitch = (
                weighted_pitch_frames[~weighted_pitch_frames.isnan()].sum()
                / weighted_window[~weighted_pitch_frames.isnan()].sum()
            ).item()
        elif pitch_sum == "weighted_median":
            weighted_window = torch.hann_window(pitch_frames.size(0))
            weighted_window[pitch_frames.isnan()] = 0
            weighted_window /= weighted_window.sum()
            pitch = wquantiles.median(
                pitch_frames.cpu().numpy(), weighted_window.cpu().numpy()
            )
