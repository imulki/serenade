...
class SSCTrainerNew(Trainer):
    ...
    def _train_step(self, batch):
        ...
        louds = batch["louds"].to(self.device)
        scores = batch["scores"].to(self.device)
        f0_flucs = batch["f0_flucs"].to(self.device)
    ...
    def _generate_and_save_intermediate_result(self, batch):
        ...
        for idx, (x, y, lns, loud, score, f0_fluc) in enumerate(
            zip(xs, ys, lens, louds, scores, f0_flucs)
        ):
            start_time = time.time()
