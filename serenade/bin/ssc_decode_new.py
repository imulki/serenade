...
def main():
    ...
    with torch.no_grad(), tqdm(dataset, desc="[decode]") as pbar:
        for idx, batch in enumerate(pbar, 1):
            ...
            f0_flucs = torch.tensor(f0_fluc, dtype=torch.float).to(device).unsqueeze(0)
            ...
            for key, ref_h5path in tqdm(
                ...
                ref_f0_fluc = read_hdf5(ref_h5path, "f0_fluc")
                ...
                ref_f0_fluc = torch.from_numpy(ref_f0_fluc).float().to(device).unsqueeze(0)
